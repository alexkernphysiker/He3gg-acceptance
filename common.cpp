...
list<LorentzVector<>> ThreePi0Decay(const LorentzVector<>&eta){
	while(true){
		...
		if((p1<0)||(p2<0)||(p3<0))throw Exception<LorentzVector<>>("Momenta error");
		...
		for(const auto PiP:pizeros){
			...
			if(!isfinite(piframe.M()))throw Exception<list<LorentzVector<>>,106>("Invalid pi0 frame");
			output.push_back(g1Pcm.Transform(-piframe));
